...
function insertAnActivityLog($pdo, $operation, $id, $first_name, 
    ...
    // Remove the $id from the insert array if it's auto-incremented
    $sql = "INSERT INTO activity_logs (operation, first_name, 
            last_name, specialization, username) VALUES(?,?,?,?,?)";

    $stmt = $pdo->prepare($sql);
    
    // Bind the variables without including $id (because it's auto-increment)
    $executeQuery = $stmt->execute([$operation, $first_name, 
                                     $last_name, $specialization, $username]);
